...
try {
    const {text} = req.body;
    let {img} = req.body;
    console.log("Incoming post data:", { text, img });
...
export const getAllPost = async(req,res)=>{
    try {
        const posts = await Post.find().sort({createdAt: -1}).populate(
            {path: "user",select:["-password","-email","-followers","-following","-link","-bio"]}
        )
        .populate({
            path:"comments.user",select:["-password","-email","-followers","-following","-link","-bio"]
        })
...
export const getLikedPosts =async(req,res)=>{
    try {
        ...
        const likedPosts = await Post.find({_id:{$in : user.likedPosts}})//$in: includes
        .populate({
            path:"user",
            select:"-password"
        })
        .populate({
            path:"comments.user",
            select:["-password","-email","-followers","-following","-link","-bio"]
        })
